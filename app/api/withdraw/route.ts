...
export async function POST(request: Request) {
  try {
    ...
    const accountPath = path.join(process.cwd(), "data", "account.json")
    ...
    const withdrawData = {
      ...
      ...(method === "crypto" &&
        walletAddress && {
          walletAddress,
          network: currency === "USDT" ? "TRC20" : currency === "BNB" ? "BSC" : "Mainnet",
        }),
    }
    ...
    await fs.writeFile(transactionsPath, JSON.stringify(transactionsData, null, 2))
